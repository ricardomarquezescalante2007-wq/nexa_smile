    ...
    try:
        if not nombre.get() or not pacientes.get():
            messagebox.showwarning(
                "Error",
                "El nombre del tratamiento y el paciente son obligatorios"
            )
            return
        ...
        messagebox.showerror(
            "Error",
            "Verifique que los datos ingresados tengan el formato correcto."
        )

    except Exception as e:
        messagebox.showerror(
            "Error",
            f"Ocurrió un error al registrar el tratamiento.\n\n{e}"
        )
...
def mostrar():
    try:
        ...
        messagebox.showerror(
            "Error",
            f"No fue posible mostrar los tratamientos.\n\n{e}"
        )
...
try:
    ...
    for i, (texto, var) in enumerate(campos, 1):
        Label(frame_form, text=texto).grid(
            row=i,
            column=0,
            sticky="e",
            padx=5,
        ...
        Entry(frame_form, textvariable=var).grid(
            row=i,
            column=1,
            padx=5,
            pady=5
